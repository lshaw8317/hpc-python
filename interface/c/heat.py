...
from _my_evolve import lib,ffi
...
def iterate(field, field0, a, dx, dy, timesteps, image_interval):
    ...
    nx,ny = field.shape
    uptr = ffi.cast("double *", ffi.from_buffer(field))
    u0ptr = ffi.cast("double *", ffi.from_buffer(field0))
    for m in range(1, timesteps+1):
        lib.evolve(uptr, u0ptr,nx,ny, a, dt, dx2, dy2)
